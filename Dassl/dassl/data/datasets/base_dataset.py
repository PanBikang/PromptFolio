...
class DatasetBase:
    ...
    def generate_fewshot_dataset(
        ...
        for data_source in data_sources:
            # generate a dict of data_source, key is the class
            ...
            for label, items in tracker.items():
                # for all class
                #  when the total data number is more than num_shots, sample num_shots data from dataset.
                #  when the total data number is less than num_shots and repeat = True, repeat some data to make the
                #    dataset have enough samples
                #  if repeat = False, then do nothing
    ...
    def generate_dirichlet_federated_dataset(
         self,*data_sources, num_shots=-1, num_users=5, beta=0.3, is_iid=False, repeat_rate = 0.0, repeat=False):
        # This code comes from datasplit.py data_partition function.
        print(f"Creating a dirichlet federated dataset")

        class_num = self.get_num_classes(data_sources[0])

        from datasplit import renormalize
        ...
        # mixed dataset
        full_dataset = np.concatenate([data_source for data_source in data_sources], axis=0)
        y = np.array([datum.label for datum in full_dataset])
        data_num = [len(data_source) for data_source in data_sources]

        n_fine_labels = class_num
        ...
        n_samples_by_client = n_samples // num_users
        ...
        # [num_user * []]
        net_dataidx_map = [[] for i in range(num_users)]
        for client_idx in range(num_users):
            label_weights = np.random.dirichlet(alpha=beta * np.ones(len(indices_by_fine_labels)))
            for ii in range(n_samples_by_client):
                fine_label_idx = np.argmax(np.random.multinomial(1, label_weights))
                label = list(indices_by_fine_labels.keys())[fine_label_idx]

                if indices_by_fine_labels[label]:
                    sample_idx = rng.choice(indices_by_fine_labels[label])
                    net_dataidx_map[client_idx].append(sample_idx)
                    indices_by_fine_labels[label].remove(sample_idx)

                if not indices_by_fine_labels[label]:
                    label_weights = np.delete(label_weights, fine_label_idx)
                    label_weights = label_weights / label_weights.sum()
                    indices_by_fine_labels.pop(label)

        # Shuffle data index map
        for data_idxs in net_dataidx_map:
            rng.shuffle(data_idxs)

        # recover the data
        data_begin = 0
        output_list = []
        for data_source_idx, data_source_num in enumerate(data_num):
            # data_num record the number of each data source
            output_dict = defaultdict(list)
            data_end = data_begin + data_source_num

            # load data for fewshot setting
            for user_idx, data_idx_map in enumerate(net_dataidx_map):
                if len(data_idx_map) >= num_shots and data_source_idx == 0:
                    sampled_idx = random.sample(data_idx_map, num_shots)
                else:
                    if repeat:
                        sampled_idx = random.choices(data_idx_map, num_shots)
                    else:
                        sampled_idx = data_idx_map

                for data_idx in sampled_idx:
                    output_dict[user_idx].append(full_dataset[data_idx])
