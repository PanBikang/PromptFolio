...
def build_data_loader(
    ...
    data_loader = torch.utils.data.DataLoader(
        dataset_wrapper(cfg, data_source, transform=tfm, is_train=is_train, classnames=classnames),
        batch_size=batch_size,
        sampler=sampler,
...
class DataManager:
    ...
    def __init__(
        ...
        if dataset.federated_train_x:
            ...
            for idx in tqdm.tqdm(range(cfg.DATASET.USERS)):
        ...
        import copy
        self.ori_train_loader_x_dict = copy.deepcopy(fed_train_loader_x_dict)
...
class DatasetWrapper(TorchDataset):
    ...
    def __init__(self, cfg, data_source, transform=None, is_train=False, classnames=None):
        self.cfg = cfg
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            if isinstance(self.transform, (list, tuple)):
                ...
                if self.cfg.TRAINER.PROMPTFL.PREC == "fp16":
                    img = img.half()
                output["img"] = img
        else:
            if self.cfg.TRAINER.PROMPTFL.PREC == "fp16":
...
def build_dataset_XU(cfg, pred=[], prob=[]):
    ...
    from dassl.utils import check_availability
